...
You should can follow the progress of the build with the following command:
...
To deploy the Risk Assessment Service, we'll be building another container image from source. To build the container image, we'll use the "oc new-app" command, as in the last example.. Enter the following command in the terminal tab:

[source,sh,role="copypaste"]
----
cd ~/machine-learning-workshop-labs/services/risk-assessment && oc new-app . --strategy=docker --name=risk-assessment
----
...
As in the previous example, the deployment will fail as the application is unable to connect to the S3 endpoint. Add the ceph-nano-credentials secret to the deployment configuration using the same steps we used with the image generator.
